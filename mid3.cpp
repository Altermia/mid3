...
double pondata[480] = {135.000000, 50.000000, 13.572088,
...
int unit_test(double tolerance) 
{
    ...
    cout << "mass" << setw(13) << "height" << setw(12) << "ponderal" << setw(10) << "returned" << endl;
    cout << "---------- --------- --------- ---------" << endl;
	cout << setw(10) << left << fixed << setprecision(6);
   for (int i = 0; i < 480; i+=3)
    {
        cout << pondata[i] << " " << pondata[i+1] << " " << pondata[i+2] 
            << " " << getPonderal(pondata[i], pondata[i+1]) << " ";
        if (pondata[i+2] - 
                getPonderal(pondata[i], pondata[i+1]) > tolerance 
                || getPonderal(pondata[i], pondata[i+1]) - 
                pondata[i+2] > tolerance) {
            cout << "<----- outside of tolerance";
            errorcount++;
        }
        cout << endl;
    }
    cout << errorcount << " errors found." << endl;
